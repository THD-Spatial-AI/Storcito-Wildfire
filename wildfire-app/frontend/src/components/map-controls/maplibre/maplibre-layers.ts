...
import { BOUNDARY_COLORS, POLYGON_COLORS, USER_MODEL_COLORS } from "./maplibre-styles";
...
export function addOrUpdateRegionBoundaries(
  ...
  const availableFC = toFeatureCollection(availableBoundaryGeoJSON) || {
    type: "FeatureCollection",
    features: [],
  };
  const selectedFC = toFeatureCollection(selectedBoundaryFeature) || {
    type: "FeatureCollection",
    features: [],
  };

  const availableSource = map.getSource(REGION_AVAILABLE_SOURCE_ID) as
  ...
  const selectedSource = map.getSource(REGION_SELECTED_SOURCE_ID) as
    | maplibregl.GeoJSONSource
    | undefined;
  ...
  map.addLayer({
    ...
    source: REGION_AVAILABLE_SOURCE_ID,
    paint: {
      "line-color": BOUNDARY_COLORS.availableStroke,
      "line-width": 1.4,
      "line-dasharray": [3, 2],
    },
  });
...
export function addOrUpdatePolygon(
  map: maplibregl.Map,
...
export function addOrUpdateUserModels(
  ...
  map.addLayer({
    id: "user-models-stroke-casing",
    type: "line",
    source: USER_MODELS_SOURCE_ID,
    paint: { "line-color": USER_MODEL_COLORS.casing, "line-width": 5.5 },
  });
  ...
  map.addLayer({
    ...
    type: "line",
    source: USER_MODELS_SOURCE_ID,
    paint: { "line-color": USER_MODEL_COLORS.stroke, "line-width": 2.5 },
  });
}
    ...
    if (features.length > 0) {
      ...
      if (id !== hoveredId) {
        ...
        map.setPaintProperty(USER_MODELS_FILL_LAYER_ID, FILL_COLOR_PAINT, [
          "case",
          ["==", ["get", "model_id"], id],
          USER_MODEL_COLORS.hoverFill,
          USER_MODEL_COLORS.fill,
        ]);
      }

      popup
        .setLngLat(e.lngLat)
        .setHTML(`<b>${props?.title || "Untitled"}</b>`)
        .addTo(map);
    } else {
      if (hoveredId !== null) {
        hoveredId = null;
...
export function setupBoundaryInteractions(
  ...
  const onMouseMove = (e: maplibregl.MapMouseEvent) => {
    const features = map.queryRenderedFeatures(e.point, {
      layers: [REGION_AVAILABLE_FILL_LAYER_ID],
    });
    if (features.length > 0) {
      ...
      if (name !== hoveredName) {
        ...
        map.setPaintProperty(REGION_AVAILABLE_FILL_LAYER_ID, FILL_COLOR_PAINT, [
          "case",
          ["==", ["get", "name"], name],
          BOUNDARY_COLORS.availableHoverFill,
          BOUNDARY_COLORS.availableFill,
        ]);
      }
      ...
      if (hoveredName) {
        ...
        map.setPaintProperty(
          REGION_AVAILABLE_FILL_LAYER_ID,
          FILL_COLOR_PAINT,
          BOUNDARY_COLORS.availableFill
        );
      }
