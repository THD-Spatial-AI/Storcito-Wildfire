...
const NOTIFICATION_ICON = '/images/logo/wildfire-app-logo.png';
...
export const useBrowserNotifications = (enabled: boolean, useInAppPanel = true) => {
  ...
  const handleNotificationEvent = useCallback((event: MessageEvent) => {
    try {
      ...
      const currentPermission = hasNotificationSupport() ? Notification.permission : 'denied';
  ...
  }, [showNotification, useInAppPanel]);
