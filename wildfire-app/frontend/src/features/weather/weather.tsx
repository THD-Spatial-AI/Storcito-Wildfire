...
const WIND_UNIT_FALLBACK: Record<string, string> = {
  kmh: "km/h",
  mph: "mph",
  ms: "m/s",
  knots: "kn",
...
const WeatherDropdown: React.FC<WeatherDropdownProps> = ({ showSettingsIcon = true }) => {
  ...
  const { temperatureUnit, windSpeedUnit } = useDataDisplayStore();
      ...
      windSpeedUnit === "mph"
        ...
        : windSpeedUnit === "ms"
          ...
          : windSpeedUnit === "knots"
            ...
            : kmh;
    const label = t(
      `settings.dataDisplay.windUnits.${windSpeedUnit}`,
      WIND_UNIT_FALLBACK[windSpeedUnit]
    );
    return `${converted.toFixed(windSpeedUnit === "ms" ? 1 : 0)} ${label}`;
  };
          ...
          <div className="absolute top-full right-0 mt-2 bg-card border border-border rounded-md shadow-xl p-4 min-w-[220px] z-50">
            <div className="flex items-center gap-2 pb-3 border-b border-border">
              ...
              <div>
                <p className="text-sm font-medium text-foreground">
                  {describeWeather(currentWeather.weather_code, currentWeather.description)}
                </p>
            ...
            <div className="grid grid-cols-1 gap-2 mt-3">
              <DetailRow
                ...
                value={`${temps.celsius}°C / ${temps.fahrenheit}°F`}
                tooltip={<p className="text-xs">{t("weather.temperatureTooltip")}</p>}
              />
              <DetailRow
                icon={<Wind className="w-4 h-4 text-cyan-500" />}
                label={t("weather.windSpeed")}
                value={formatWindSpeed(currentWeather.wind_speed)}
                tooltip={
                  <p className="text-xs">
                    {t("weather.windSpeedTooltip", "Change the unit in Settings → Data display.")}
                  </p>
                }
              />
              <DetailRow
                ...
                tooltip={<p className="text-xs">{t("weather.humidityTooltip")}</p>}
              />
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="flex items-center justify-between p-2 rounded-md hover:bg-muted cursor-pointer transition-colors">
                    <div className="flex items-center gap-2">
                      <SunDim
                        className={cn(
                          "w-4 h-4",
                          uvLevel == null ? "text-gray-400 dark:text-gray-300" : uvLevel.color
                        )}
                      />
                    ...
                    {uvLevel == null ? (
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-foreground">
                          {Math.round(currentWeather.uv_index ?? 0)}
                        </span>
                      </div>
                    )}
                  </div>
