...
const parseBooleanSetting = (value: unknown, fallback = true): boolean => {
...
class SettingsService {
  ...
  async getNotificationPreferences(): Promise<{ email: boolean; browser: boolean }> {
    try {
      ...
      return {
        email: parseBooleanSetting(rawSettings.email_notifications, true),
        browser: parseBooleanSetting(rawSettings.browser_notifications, true),
      };
    } catch (e) {
  ...
  async setModelIntroCardDismissed(dismissed: boolean): Promise<boolean> {
    try {
      ...
      if (import.meta.env.DEV) console.error('Failed to save model intro preference:', e);
      return false;
    }
  }
  ...
  async markProductTourCompleted(): Promise<boolean> {
    try {
      ...
      if (import.meta.env.DEV) console.error('Failed to save tour completion status:', e);
      return false;
    }
  }
