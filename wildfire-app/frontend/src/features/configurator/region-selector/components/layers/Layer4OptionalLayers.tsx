...
export const Layer4OptionalLayers: FC<{ ctx: ConfiguratorContext }> = ({ ctx }) => {
    ...
    const ITEMS: OptionalLayerItem[] = [
        {
            ...
            hint: t("configurator.layer3.fwiHint", "Wind, humidity, temperature, drought — the strongest fire driver"),
            icon: CloudRain,
        ...
        },
        {
            ...
            hint: t("configurator.layer3.historicalHint", "Recurrence based on past fire incidents in the area"),
            icon: Flame,
    ...
    return (
        <LayerShell
            purpose={t("configurator.layer3.purpose", "These signals feed the AHP-weighted risk model. All three are enabled by default and recommended for an accurate forest-fire risk assessment.")}
            nextStepHint={t("configurator.layer3.nextStepHint", "Next we'll do a final review before saving.")}
        >
            ...
            {weatherDisabled && (
                <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-500/40 bg-amber-500/10 px-3 py-2.5 text-[11px] leading-snug text-amber-700 dark:text-amber-300">
                    ...
                    <span>
                        {t("configurator.layer3.weatherWarning", "Fire-weather is the main risk signal. Disabling it makes the date selection irrelevant and produces a drastically under-estimated risk map. Keep it on unless you specifically need a baseline without weather.")}
                    </span>
                </div>
            )}

            <div className="mt-4 border-t border-border pt-4">
                <h4 className="text-xs font-semibold text-foreground">{t("configurator.layer3.customData", "Custom data (optional)")}</h4>
                <p className="mb-2 mt-0.5 text-[11px] leading-snug text-muted-foreground">
                    {t("configurator.layer3.customDataDesc", "Bring your own weather inputs for this area. If left empty, the bundled regional data is used.")}
                ...
                <div className="space-y-2">
                    <FileUploadField
                        ...
                        accept=".xlsx,.xls,.csv"
                        hint={t("configurator.layer3.weatherStationHint", "Excel or CSV station export (.xlsx / .csv)")}
                        info={t("configurator.layer3.weatherStationInfo", "Hourly weather measurements from a local station — temperature, humidity, wind and precipitation. Used to compute the Fire Weather Index for this area. Upload an Excel or CSV export; if omitted, the bundled regional weather data is used.")}
                        icon={CloudRain}
