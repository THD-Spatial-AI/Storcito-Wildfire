...
const MapSearchBar: React.FC<{ className?: string }> = ({ className }) => {
  ...
  const selectResult = (r: GeocodingResult) => {
    ...
    if (r.geojson && r.geojson.type !== 'Point') {
  ...
  return (
    <div
      className={className ?? "absolute top-4 z-50 pointer-events-auto flex justify-end"}
      style={className ? undefined : { right: "calc(1rem + var(--sidebar-offset, 0rem))" }}
      ref={containerRef}
      data-tour="municipality-search"
    >
      {isExpanded ? (
        // Expanded - Search Bar
            ...
            <div className="border border-border rounded-md max-h-60 overflow-auto divide-y divide-border bg-popover text-popover-foreground">
              {results.map((r) => (
                <button
                  type="button"
                  key={r.id}
                  className="w-full text-left px-3 py-2 hover:bg-muted text-sm transition-colors text-foreground truncate"
        ...
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              ...
              className="flex items-center justify-center size-8 rounded-full bg-secondary text-secondary-foreground shadow-sm hover:bg-secondary/80 transition-colors border border-border"
            >
