...
func (p *TaskProcessor) ProcessTask(ctx context.Context, t *asynq.Task) error {
	...
	case jobs.TypeProcessResult:
		return jobs.HandleProcessResult(ctx, t, p.db, p.notificationService, p.wsClient, p.geoClient)
...
func (p *TaskProcessor) ServeMux() *asynq.ServeMux {
	mux := asynq.NewServeMux()
	mux.HandleFunc(jobs.TypeBroadcastNotification, p.ProcessTask)
	mux.HandleFunc(jobs.TypeProcessResult, p.ProcessTask)
	mux.HandleFunc(jobs.TypeDomainEvent, p.ProcessTask)
	return mux
}
