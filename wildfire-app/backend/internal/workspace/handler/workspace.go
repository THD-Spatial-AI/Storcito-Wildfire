...
func NewWorkspaceHandler(db *gorm.DB, adminTokenProvider *authplatform.AdminTokenProvider, keycloakBaseURL, realm string, kcCache *cache.KeycloakCacheService) *WorkspaceHandler {
	return NewWorkspaceHandlerWithService(workspaceservice.NewWorkspaceService(db, adminTokenProvider, keycloakBaseURL, realm, kcCache))
}

func NewWorkspaceHandlerWithService(service services.WorkspaceService) *WorkspaceHandler {
