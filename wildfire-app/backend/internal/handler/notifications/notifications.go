...
func NewHandlerWithAsynqAndRedis(db *gorm.DB, asynqClient *asynq.Client, redisClient *goredis.Client) *Handler {
	return NewHandlerWithStore(notificationStore.NewStore(db), asynqClient, redisClient)
}

func NewHandlerWithStore(store services.NotificationStore, asynqClient *asynq.Client, redisClient *goredis.Client) *Handler {
	return &Handler{
		store:       store,
