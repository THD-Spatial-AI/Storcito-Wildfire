...
// HasModelShare reports whether a direct model share grants this user access.
// Email matching supports invitations created before the recipient's user ID
// was known or backfilled.
func (s *Store) HasModelShare(modelID uint, userID, email string) (bool, error) {
	var count int64
	query := s.db.Model(&commonModels.ModelShare{}).Where("model_id = ?", modelID)
	if email != "" {
		query = query.Where("user_id = ? OR LOWER(email) = LOWER(?)", userID, email)
	} else {
