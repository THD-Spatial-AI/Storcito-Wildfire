...
func (h *ModelHandler) newModelService() *modelservice.ModelService {
	if h.keycloakCache != nil {
		return modelservice.NewModelServiceWithStoreAndCache(h.store, h.kc, h.keycloakCache)
	}
	return modelservice.NewModelServiceWithStore(h.store, h.kc)
}
...
func (h *ModelHandler) ensureWorkspaceAccess(c *gin.Context, userID string, workspaceID uint, message string) bool {
	...
	if h.authz != nil {
		if err := h.authz.CanAccessWorkspace(c.Request.Context(), userCtx, workspaceID); err == nil {
			return true
		}
	} else if h.newModelService().UserHasWorkspaceAccessWithEmail(userID, userCtx.Email, workspaceID) {
		return true
	}
	if message == "" {
